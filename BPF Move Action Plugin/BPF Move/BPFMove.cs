    ...
    public class BPFMove : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            {               
                ...
                var id = context.InputParameters["EntityId"].ToString().Replace("{", string.Empty).Replace("}", string.Empty);
                entityid = new Guid(id);
                tracingService.Trace("name : " + name);
                tracingService.Trace("id : " + entityid);
                if (entityid != null)
                {
                    Entity entity2 = new Entity("cr224_actioncustom2");
                    entity2["cr224_name"] = name;
                    entity2["cr224_actioncustom1"] = new EntityReference("cr224_actioncustom1", entityid);
                ...
                context.OutputParameters["output"] = entity2Id;

            }
            catch (Exception ex)
            {
                context.OutputParameters["output"] = "error";
